...
function decodeBC3(imageData, width, height, premultiplied) {
    ...
    for (h = 0; h < height_4; h++) {
        for (w = 0; w < width_4; w++) {
            alphaValues = utils.interpolateAlphaValues(
            ...
            colorValues = utils.interpolateColorValues(
            ...
            for (y = 0; y < 4; y++) {
                for (x = 0; x < 4; x++) {
                    ...
                    rgba[rgbaIndex + 1] = utils.multiply(
                        colorValues[colorIndex * 4 + 1],
                        multiplier
                    );
                    rgba[rgbaIndex + 2] = utils.multiply(
                        colorValues[colorIndex * 4 + 2],
                        multiplier
                    );
